...
const CreateInvoice = () => {
  ...
  const [notes, setNotes] = useState(
    "1. All invoices are due and payable advance or on due date.\n" +
    "2. The invoice reflects the agreed-upon services as outlined in the proposal or contract.\n" +
    "3. Any additional services not covered in the initial agreement will be subject to additional charges.\n" +
    "4. Full advance payment of subscription is required to commence services of social media management."
  );
    ...
    <DashboardLayout>
      <div className={styles.pageHeader}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          <div>
            <h1 className={styles.title}>Create {invoiceType}</h1>
            <p className={styles.subtitle}>Enter invoice billing details and line items</p>
          </div>
          {nextInvoiceNumber && (
      ...
      <Card style={{ padding: '2rem' }}>
        <form onSubmit={handleSubmit}>
          ...
          {/* Invoice Date, Terms, and Due Date Row */}
          <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '1.5rem', flexWrap: 'wrap', alignItems: 'center', maxWidth: '900px' }}>
            {/* Invoice Date */}
            <div style={{ display: 'flex', flex: 1, alignItems: 'center', gap: '1rem', minWidth: '220px' }}>
              <label style={{ fontSize: '0.875rem', fontWeight: 600, color: '#FF9F43', whiteSpace: 'nowrap', margin: 0 }}>Invoice Date <span style={{ color: '#EA5455' }}>*</span></label>
              <input 
                type="date" 
                value={invoiceDate}
            ...
            {/* Due Date */}
            <div style={{ display: 'flex', flex: 1, alignItems: 'center', gap: '1rem', minWidth: '220px' }}>
              <label style={{ fontSize: '0.875rem', fontWeight: 600, color: '#4B5563', whiteSpace: 'nowrap', margin: 0 }}>Due Date</label>
              <input 
                type="date" 
                value={dueDate}
          ...
          <div style={{ border: '1px solid #E5E7EB', borderRadius: '8px', overflow: 'hidden', marginBottom: '1.5rem' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              ...
              <tbody>
                {cartItems.map((item) => (
                  <tr key={item.product._id} style={{ borderBottom: '1px solid #E5E7EB', fontSize: '0.875rem' }}>
                    <td style={{ padding: '0.75rem 1rem', color: '#1B2850', fontWeight: 500 }}>{item.product.name}</td>
                    ...
                    <td style={{ padding: '0.75rem 1rem' }}>
                      <input 
                        ...
                        onChange={(e) => {
                          ...
                          setCartItems(cartItems.map(ci => ci.product._id === item.product._id ? { ...ci, price: val } : ci));
                        }}
                      />
                    </td>
          ...
          <div style={{ display: 'flex', gap: '2rem', marginTop: '2rem', marginBottom: '2rem', flexWrap: 'wrap', alignItems: 'stretch' }}>
            ...
            <div style={{ flex: 1, minWidth: '320px', maxWidth: '540px', backgroundColor: '#F9FAFB', padding: '1.5rem', borderRadius: '8px', border: '1px solid #E5E7EB' }}>
              ...
              {/* Tax */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.65rem 0', borderBottom: '1px solid #E5E7EB' }}>
                <span style={{ color: '#4B5563', fontSize: '0.875rem', fontWeight: 500 }}>Tax Amount</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <input 
                    ...
                    className={formStyles.input}
                    style={{ width: '120px', margin: 0, textAlign: 'right', fontSize: '0.875rem' }}
                  />
                </div>
              </div>
              ...
              {/* Discount */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.65rem 0', borderBottom: '1px solid #E5E7EB' }}>
                <span style={{ color: '#4B5563', fontSize: '0.875rem', fontWeight: 500 }}>Discount Amount</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{ fontSize: '0.875rem', color: '#4B5563' }}>₹</span>
                  <input 
                    type="number" 
              ...
              {/* Shipping */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.65rem 0', borderBottom: '1px solid #E5E7EB' }}>
                <span style={{ color: '#4B5563', fontSize: '0.875rem', fontWeight: 500 }}>Shipping Charges</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{ fontSize: '0.875rem', color: '#4B5563' }}>₹</span>
                  <input 
                    type="number" 
              ...
              {/* Grand Total - Block Row with Gray Background */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', margin: '0.5rem -1.5rem', backgroundColor: '#EAECEF', fontWeight: 'bold' }}>
              ...
              {/* Amount Paid - Blue Highlight Box */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 0', borderBottom: '1px solid #E5E7EB' }}>
                <span style={{ color: '#4B5563', fontSize: '0.875rem', fontWeight: 500 }}>Amount Paid</span>
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  <span style={{ position: 'absolute', left: '0.75rem', color: '#1B2850', fontWeight: 600, fontSize: '0.875rem' }}>₹</span>
                  <input
                    type="number"
                    min="0"
                    max={grandTotal}
              ...
              <div style={{ padding: '0.75rem 0', borderBottom: '1px solid #E5E7EB' }}>
                <div style={{ fontWeight: 600, color: '#1B2850', fontSize: '0.875rem', marginBottom: '0.5rem' }}>Select Payment Method</div>
                  ...
                  {['Cash', 'Card', 'UPI', 'Bank Transfer'].map((pm) => (
                    <button
                      ...
                      style={{
                        ...
                        backgroundColor: paymentMethod === pm ? '#E8F9EE' : 'white',
                        color: paymentMethod === pm ? '#1B2850' : '#4B5563',
                        fontWeight: 600,
                        borderRadius: '6px',
                        cursor: 'pointer',
                        fontSize: '0.875rem',
              ...
              {/* Balance Due */}
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.65rem 0', borderBottom: '1px solid #E5E7EB' }}>
                <span style={{ color: '#4B5563', fontSize: '0.875rem', fontWeight: 500 }}>Balance Due</span>
                <span style={{ color: '#EA5455', fontSize: '0.875rem', fontWeight: 600 }}>₹{dueAmt.toFixed(2)}</span>
              </div>

              {/* Status */}
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.65rem 0' }}>
                <span style={{ color: '#4B5563', fontSize: '0.875rem', fontWeight: 500 }}>Status</span>
                <span style={{ 
                  color: payStatus === 'Paid' ? '#28C76F' : (payStatus === 'Partial' ? '#FF9F43' : '#EA5455'), 
                  fontSize: '0.875rem', 
                  fontWeight: 600 
                }}>{payStatus}</span>
              </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', borderTop: '1px solid #E5E7EB', paddingTop: '1.5rem' }}>
            ...
            <button type="submit" className={formStyles.btnSubmit} disabled={loading} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Save size={18} /> {loading ? 'Saving...' : `Save ${invoiceType}`}
            </button>
          </div>
        </form>
      </Card>
