  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/company-settings`)
      ...
      .then(parsed => {
        if (parsed) {
          setBilledBy({
            name: parsed.orgName || 'vidisha pvt ltd',
            address: `${parsed.orgAddress1 || ''}, ${parsed.orgAddress2 || ''}, ${parsed.orgCity || ''}, ${parsed.orgState || ''} - ${parsed.orgPincode || ''}`.replace(/(,\s*){2,}/g, ', ').trim().replace(/(^,\s*|,\s*$)/g, '') || 'Madhya Pradesh, India',
            gstin: parsed.orgGst || '23AAQCM8058H2Z1',
            pan: parsed.storePan || 'AAQCM8058H'
          });
          if (parsed.orgLogo) {
            setBusinessLogo(parsed.orgLogo);
          } else {
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ backgroundColor: '#FF9F43', color: 'white', fontSize: '0.875rem' }}>
                  ...
                  <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontWeight: 600, width: '110px' }}>Rate</th>
                  {isItemWiseDiscountActive && (
                    <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontWeight: 600, width: '120px' }}>Discount</th>
                  )}
                  <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontWeight: 600, width: '110px' }}>Amount</th>
                  <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontWeight: 600, width: '90px' }}>CGST</th>
                  <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontWeight: 600, width: '90px' }}>SGST</th>
                  <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontWeight: 600, width: '110px' }}>Total</th>
                </tr>
              </thead>
              <tbody>
                {cartItems.map((item, index) => (
                  <React.Fragment key={item.id}>
                    <tr style={{ borderBottom: '1px solid #E5E7EB', fontSize: '0.85rem', verticalAlign: 'middle' }}>
                      ...
                      {/* Amount before tax */}
                      <td style={{ padding: '1rem 0.5rem', color: '#374151', fontWeight: 500 }}>
                        ₹{(
                          (() => {
                            const amountBeforeDiscount = Number(item.qty) * Number(item.price);
                            const discVal = item.discountRate ? (item.discountType === '%' ? amountBeforeDiscount * (Number(item.discountRate) / 100) : Number(item.discountRate)) : 0;
                            return Math.max(0, amountBeforeDiscount - discVal);
                          })()
                        ).toFixed(2)}
                      </td>
                      ...
                      {/* Line Total */}
                      <td style={{ padding: '1rem 0.5rem', color: '#111827', fontWeight: 600 }}>
