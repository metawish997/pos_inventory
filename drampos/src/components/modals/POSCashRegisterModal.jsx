...
const POSCashRegisterModal = ({ isOpen, onClose }) => {
  ...
  const details = [
    { label: 'Register Opening Balance', value: `₹0` },
    { label: 'Total Sale Amount', value: `₹${totalSales}` },
    { label: 'Total Payments Received', value: `₹${totalSales + totalIncomes}` },
    { label: 'Total Expenses Paid Out', value: `₹${totalExpenses}` }
  ];
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Cash Register Details" maxWidth="500px">
      {loading ? (
        <div style={{ padding: '2rem', textAlign: 'center' }}>Loading Register Status...</div>
      ) : (
        <>
          <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid #E5E7EB', borderRadius: '4px', overflow: 'hidden', marginBottom: '1.5rem' }}>
            ...
            <div style={{ 
              ...
              <span>Closing Cash in Register</span>
              <span>₹{totalCash}</span>
            </div>
          </div>
